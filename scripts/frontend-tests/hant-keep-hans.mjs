...
export const TOOL_MISCONVERT = Object.freeze({
  ...
  "90 天后": {
    ...
    wrong: "90 天后",
    why:
      "第三个实例,而且**成因跟前两条相反** —— 前两条是「词组表没收 → 落回单字默认」," +
      "这条是「词组表**收错了**」:「天后」在表里是神明/歌后那个词,于是整句里的" +
      "「90 天后自动删除」原样不转。对照:`s2hk(\"三天后\")` 转得对(= 三天後)," +
      "差别只在数字与「天」之间有没有空格。" +
      "出现在登录页 PICS 个人资料收集声明里(凭证照片保存 90 天後自动删除)—— " +
      "香港 PDPO 第486章下的告示,是港方用户看到的第一屏。" +
      "⚠️ 它长在 `scripts/login-page.html` 而不是 `frontend-overrides/`," +
      "所以守卫的干草堆专门为它加了登录页那一半(见 hant-ui-strings.test.ts 里的说明)。",
  },
});
