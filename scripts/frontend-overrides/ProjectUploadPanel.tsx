 ...
 *   不写死设计稿里的示例名。
 *
 * 视觉 = 方案 B(浅灰绿底、纯白卡片、大圆角、单一绿 #16805C),信息架构用 ①存到哪个工地 →
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className="mx-auto flex h-full w-full max-w-[960px] flex-col overflow-hidden rounded-[20px] bg-white shadow-[0_1px_2px_rgba(23,28,26,0.05),0_24px_60px_rgba(23,28,26,0.12)]"
      >
        <div className="flex items-center gap-3 border-b border-[#EAEDEB] px-5 py-3">
          {/* title 是用户自己起的图纸/文件名,原样上屏、不转(与顶栏 chip 同一条规矩)。 */}
          <div className="truncate text-[15px] font-bold text-[var(--gyt-ink)]">{preview.title}</div>
          <button
            onClick={closePreview}
            className="ml-auto text-[22px] leading-none text-[var(--gyt-muted)] hover:text-[var(--gyt-muted)]"
            title="關閉預覽"
          >
            ✕
          </button>
...
function ProjectSwitcher() {
  ...
  const rowCls = (active: boolean) =>
    cn(
      ...
      active ? "font-bold text-[var(--gyt-ink)]" : "text-[var(--gyt-muted)]",
    );
    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={cn(
          ...
          scoped
            ? "border-[var(--gyt-green)] bg-[var(--gyt-soft)] text-[var(--gyt-green-deep)]"
            : "border-dashed border-[#C6D0CB] bg-white text-[var(--gyt-muted)] hover:border-[var(--gyt-mint)]",
        )}
      ...
      >
        <span
          className={cn(
            ...
            scoped ? "bg-[var(--gyt-green)]" : "bg-[#C6D0CB]",
          )}
        />
        ...
        <span className="shrink-0 text-[var(--gyt-muted)]">▾</span>
      </button>

      {menuOpen && (
        <div className="absolute right-0 z-50 mt-2 w-[248px] overflow-hidden rounded-[14px] border border-[var(--gyt-line)] bg-white shadow-[0_12px_40px_rgba(27,36,32,0.16)]">
          <div className="px-3 pt-2.5 pb-1 text-[12px] font-bold text-[var(--gyt-muted)]">切換當前工地</div>
          <button
            onClick={() => {
              setProjectId("");
              setMenuOpen(false);
          ...
          {projects.map((p) => (
            ...
            >
              <span
                className={cn(
                  "h-2 w-2 shrink-0 rounded-full",
                  p.id === projectId ? "bg-[var(--gyt-green)]" : "bg-[#C9D2CD]",
                )}
              />
              <span className="truncate">{p.name}</span>
              <span className="ml-auto shrink-0 text-[12px] text-[var(--gyt-muted)]">{p.id}</span>
            </button>
          ))}
          <button
            onClick={() => {
            ...
            className="flex w-full items-center gap-2 border-t border-[#EEF1F0] px-3 py-2.5 text-[14px] font-bold text-[var(--gyt-green-deep)] transition hover:bg-[#F7F9F8]"
          >
            ＋ 新建 / 管理工地
          </button>
        </div>
  ...
  return (
    ...
    <div className="flex shrink-0 items-center gap-1.5 sm:gap-2.5">
      ...
      <button
        ...
        // 🔴 2026-08-25 设计审计:这颗原本是 `bg-[var(--gyt-ink)]` 纯黑实心,**是整个顶栏
        ...
        className="flex shrink-0 items-center gap-2 rounded-full border border-[#CBD5D0] bg-[#F1F4F3] px-2.5 py-2.5 text-[14px] font-bold whitespace-nowrap text-[var(--gyt-ink)] transition hover:border-[var(--gyt-mint)] hover:bg-white sm:px-4"
...
function ArchiveDrawer() {
  ...
  // --- 样式片段(方案 B 调性)---------------------------------------------------
  const stepLabel = "mb-2.5 text-[13px] font-bold text-[var(--gyt-green-deep)]";
  const fieldCls =
    "w-full rounded-[14px] border border-[var(--gyt-line)] bg-white px-4 py-3.5 text-[15px] text-[var(--gyt-ink-soft)] placeholder:text-[var(--gyt-muted)] focus:border-[var(--gyt-green)] focus:outline-none";
  const toggle = (active: boolean) =>
    "rounded-[12px] py-3.5 text-center text-[16px] font-bold transition " +
    (active
      ? "bg-[var(--gyt-green)] text-white"
      : "border border-[var(--gyt-line)] bg-white text-[var(--gyt-muted)] hover:border-[var(--gyt-mint)]");
  const bigChoice = (active: boolean) =>
    "rounded-[14px] p-4 text-center transition " +
    (active
      ? "border-2 border-[var(--gyt-green)] bg-[var(--gyt-soft)]"
      : "border border-[var(--gyt-line)] bg-white hover:border-[var(--gyt-mint)]");
  const archiveBtn =
    "rounded-full bg-[var(--gyt-green)] py-4 text-[19px] font-black text-white transition hover:bg-[var(--gyt-green-deep)] disabled:opacity-50";
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-[#EAEDEB] bg-white px-7 py-5">
          <div className="flex h-[34px] w-[34px] items-center justify-center rounded-[10px] bg-[var(--gyt-soft)] text-lg">
            📂
          </div>
          <div>
            <div className="text-[19px] font-black text-[var(--gyt-ink)]">資料歸檔</div>
            <div className="text-[13px] text-[var(--gyt-muted)]">把圖紙、規範正式存進項目</div>
          </div>
          <button
            onClick={() => setOpen(false)}
            className="ml-auto text-[22px] leading-none text-[var(--gyt-muted)] hover:text-[var(--gyt-muted)]"
          ...
          <section>
            ...
            <div className="flex gap-2.5">
              <select
                ...
                className={fieldCls + " font-bold text-[var(--gyt-ink)]"}
              >
              ...
              <button
                ...
                className="whitespace-nowrap rounded-[14px] border border-dashed border-[var(--gyt-mint)] bg-[var(--gyt-soft)] px-5 py-3.5 text-[16px] font-bold text-[var(--gyt-green-deep)]"
              >
                ＋ 新建
              </button>
            </div>
            {showNew && (
              <div className="mt-2.5 flex gap-2">
                ...
                />
                <input
                  className="w-[90px] rounded-[14px] border border-[var(--gyt-line)] bg-white px-3 py-3.5 text-[15px]"
                  placeholder="短碼"
                  value={newCode}
                ...
                <button
                  ...
                  disabled={busy}
                  className="rounded-[14px] bg-[var(--gyt-green)] px-4 text-[15px] font-bold text-white disabled:opacity-50"
                >
                  建
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button onClick={() => setTab("drawing")} className={bigChoice(tab === "drawing")}>
                <div
                  className={
                    ...
                    (tab === "drawing" ? "text-[var(--gyt-green-deep)]" : "text-[var(--gyt-muted)]")
                  }
                >
                  📐 圖紙
                </div>
              ...
              <button onClick={() => setTab("doc")} className={bigChoice(tab === "doc")}>
                <div
                  className={
                    "text-[18px] font-black " +
                    (tab === "doc" ? "text-[var(--gyt-green-deep)]" : "text-[var(--gyt-muted)]")
                  }
                >
                  📄 資料
                </div>
          ...
          {tab === "drawing" ? (
            <section className="flex flex-col gap-3">
              ...
              <label
                ...
                className="block cursor-pointer rounded-[16px] border-2 border-dashed border-[#C6D0CB] bg-white px-6 py-6 text-center text-[16px] text-[var(--gyt-muted)]"
              >
                <input
                  type="file"
                  accept=".dxf,.pdf"
                ...
                />
                {dwgFile ? (
                  <span className="font-bold text-[var(--gyt-ink)]">{dwgFile.name}</span>
                ) : (
                  <>
                    拖入 .dxf / .pdf,或 <span className="font-bold text-[var(--gyt-green-deep)]">點擊選擇</span>
                  </>
                )}
              </label>

              <div className="text-[14px] font-semibold text-[var(--gyt-muted)]">這是哪種圖?</div>
              ...
              <div className="flex gap-2.5">
                ...
                />
                <input
                  className="w-[130px] rounded-[14px] border border-[var(--gyt-line)] bg-white px-4 py-3.5 text-[15px] placeholder:text-[var(--gyt-muted)]"
                  placeholder="樓層·可選"
                  value={floor}
            ...
            <section className="flex flex-col gap-3">
              ...
              <div className="text-[14px] font-semibold text-[var(--gyt-muted)]">作用域</div>
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setDocScope("global")}
                  className={bigChoice(docScope === "global")}
                >
                  <div className="text-[16px] font-black text-[var(--gyt-ink)]">全局規範</div>
                  <div className="mt-1 text-[12px] font-bold text-[var(--gyt-muted)]">所有項目通用</div>
                </button>
                <button
                  onClick={() => setDocScope("project")}
                  className={bigChoice(docScope === "project")}
                >
                  <div className="text-[16px] font-black text-[var(--gyt-ink)]">本項目</div>
                  <div className="mt-1 text-[12px] font-bold text-[var(--gyt-muted)]">
                    {currentName ?? "先選項目"}
                  </div>
                </button>
              </div>
              ...
              >
                ...
                />
                {docFile ? (
                  <span className="font-bold text-[var(--gyt-ink)]">{docFile.name}</span>
                ) : (
                  <>
                    拖入 .pdf,或 <span className="font-bold text-[var(--gyt-green-deep)]">點擊選擇</span>
                  </>
                )}
              </label>
...
function RowDelete({
  ...
  if (busy) return <span className="text-[12px] text-[var(--gyt-muted)]">刪除中…</span>;
  if (confirming) {
    return (
      <span className="flex items-center gap-2 text-[12px]">
        ...
        <button
          ...
          className="text-[var(--gyt-muted)] hover:underline"
        >
          取消
        </button>
      </span>
  ...
  return (
    <button
      ...
      className="text-[12px] font-bold text-[var(--gyt-muted)] transition hover:text-[#C0392B]"
      title={label}
    >
      {label}
    </button>
...
export function LibraryButton() {
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 border-b border-[#EAEDEB] bg-white px-7 py-5">
          ...
          <div>
            <div className="text-[19px] font-black text-[var(--gyt-ink)]">資料庫</div>
            <div className="text-[13px] text-[var(--gyt-muted)]">所有項目的圖紙與規範</div>
          </div>
          <button
            onClick={() => void load()}
            className="ml-auto rounded-[10px] border border-[var(--gyt-line)] px-3 py-1.5 text-[13px] font-bold text-[var(--gyt-muted)] transition hover:border-[var(--gyt-mint)]"
            title="刷新"
          >
            刷新
          </button>
          <button
            onClick={() => setOpen(false)}
            className="text-[22px] leading-none text-[var(--gyt-muted)] hover:text-[var(--gyt-muted)]"
          >
            ✕
          </button>
        </div>
  ...
  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className="flex shrink-0 items-center gap-2 rounded-full border border-[var(--gyt-line)] bg-white px-2.5 py-2 text-[14px] font-bold whitespace-nowrap text-[var(--gyt-ink-soft)] transition hover:border-[var(--gyt-mint)] sm:px-4"
        title="查看所有項目的圖紙與規範"
...
function LibraryBody({ data, reload }: { data: LibraryData; reload: () => Promise<void> }) {
  ...
  if (empty) {
    return (
      <div className="rounded-[16px] border border-dashed border-[#C6D0CB] bg-white px-6 py-12 text-center text-[15px] leading-relaxed text-[var(--gyt-muted)]">
        還沒有任何圖紙或資料。
        <br />
        點右上角「📂 資料歸檔」傳第一份吧。
      </div>
        ...
        return (
          ...
          >
            {dwgs.length === 0 && docs.length === 0 ? (
              <div className="px-1 py-1.5 text-[13px] text-[var(--gyt-muted)]">（暫無圖紙或資料)</div>
            ) : (
              <>
                {dwgs.map((d) => (
  ...
  return (
    <section>
      <div className="mb-2 flex items-baseline gap-2">
        <div className="text-[15px] font-black text-[var(--gyt-ink)]">{title}</div>
        <div className="text-[12px] text-[var(--gyt-muted)]">{hint}</div>
        <div className="ml-auto flex items-baseline gap-3">
          {action}
          <div className="rounded-full bg-[var(--gyt-soft)] px-2.5 py-0.5 text-[12px] font-bold text-[var(--gyt-green-deep)]">
            {count}
          </div>
        </div>
      </div>
...
function ActionBtn({
  ...
  return (
    <button
      ...
      className="shrink-0 rounded-[8px] border border-[var(--gyt-line)] px-2.5 py-1 text-[12px] font-bold text-[var(--gyt-ink-soft)] transition hover:border-[var(--gyt-mint)] disabled:opacity-50"
    >
      {busy ? "…" : label}
    </button>
  );
  ...
  return (
    <div className="flex items-center gap-2 rounded-[14px] border border-[#EAEDEB] bg-white px-4 py-3">
      ...
      <div className="min-w-0">
        <div className="truncate text-[15px] font-bold text-[var(--gyt-ink)]">{dwg.title}</div>
        <div className="text-[12px] text-[var(--gyt-muted)]">圖紙{dwg.floor ? ` · ${dwg.floor}` : ""}</div>
      </div>
      <span className="ml-auto shrink-0 rounded-full bg-[var(--gyt-soft)] px-2.5 py-1 text-[12px] font-bold text-[var(--gyt-green-deep)]">
...
function DocStatus({ doc }: { doc: LibDoc }) {
  if (doc.status === "indexed" || (doc.status !== "unsearchable" && doc.chunks > 0)) {
    return <span className="text-[var(--gyt-green-deep)]">已入庫 {doc.chunks} 段</span>;
  }
...
function DocRow({ doc, reload }: { doc: LibDoc; reload: () => Promise<void> }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-[14px] border border-[#EAEDEB] bg-white px-4 py-3">
      ...
      <div className="min-w-0">
        <div className="truncate text-[15px] font-bold text-[var(--gyt-ink)]">{doc.filename}</div>
        <div className="text-[12px] text-[var(--gyt-muted)]">
