...
/**
 * 🔴 **2026-09-18 起這張表不再自己維護 —— 它就是 `timing-lib.AGENT_LABELS`**
 *    (設計審查 FINDING-005)。在此之前同一個能力在界面上有 9 個名字:狀態卡叫「識隱患」、
 *    這裏叫「安全巡檢員」/「巡檢出記錄」/「報告生成」、耗時行叫「識隱患」、調度者一邊叫
 *    「調度中樞」一邊叫「調度」—— 一次拍照工友按順序看到 6 個名字,以為是 6 個東西在跑。
 *    `AGENT_LABELS` 那邊有一條**讀 GytStatusCards 源碼比對**的守衛,所以三處從此只會一起變。
 *    英雄鏈的 safety / inspection / report 在工友眼裏就是「識隱患」這一件事,三個鍵同一個名。
 */
export const AGENT_NAMES: Readonly<Record<string, string>> = AGENT_LABELS;
...
/**
 * 巡检记录卡片 —— 「拍照 → 自动出 Word」这条链本来唯一看得见的产出。
 *
 ...
 *
 * ───────────────────────────────────────────────────────────────────────────
 * 🔴 **墓碑(2026-08-22):这张卡从 W3 上线起一次都没渲染出来过,而它仍然留着。**
 * ───────────────────────────────────────────────────────────────────────────
 * 上面那段推理只有一个前提是错的:**这张卡的输入压根到不了这儿**。
 * 它的判据是「聊天流里有一条 `render_inspection_report` 的 ToolMessage」,
 * 而 supervisor 的 `output_mode="last_message"` 会把子 Agent 的工具返回整个丢掉 ——
 * 与 W9 那张隐患台账卡是**同一个根因**(`docs/W10_界面取不到工具返回_方案.md`)。
 * 于是「拍照 → 自动出 Word」这条链的终点一直是断的:文件在盘上,谁都拿不到;
 * 而 `report/prompt.md` 当时教模型说「跟管理员说编号就行」——**那个管理员不存在**。
 *
 * **出口已经另建**:动作条上的「記錄」按钮 → `GET /reports` → 直接下载
 * (`reports-entry.tsx` + `backend/src/gyt/reports_api.py`),形状照 W7 打卡、
 * W10 监理操作台的先例:**操作台不是聊天产物,它有自己的入口和自己的数据源**。
 *
 * 这张卡**刻意留着并标注不可达**,与 `HazardIntakeCard` 同一条规矩:
 * 静默的死代码比标注过的死代码危险。哪天有人改掉 `output_mode`,
 * 它会自己活过来 —— 那时它是对的,不用重写。
 */
