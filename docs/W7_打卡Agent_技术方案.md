...
> 仍不在本轮范围内,**但方向已定并记在 TODO-39**:打卡侧**不加项目下拉**,
> 用已经收着的坐标反查工地(`POST /checkin` 不需要新增任何 header)——
> 工地上少打一个字就少一个不用它的理由,而且「手打名对不上建档名」从根上没了。
> 前置是 `projects` 表补坐标。两处实测出来的坑也记在那儿:**拿不到坐标不是边缘情况**
> (真机三次就有一次 timeout),以及 **±35 米撑不住香港市区相邻地盘**。
