...
> **W2 开工前三个前置(见 W2 计划 §02):** ① 收口 TODO-5(缓存/重试不在真实执行路径,
> 「断网兜底」目前是空的);② 搭评测跑分框架 `backend/eval/`(共享基座,你搭队友用 ——
> 各写各的会得到两套不可横向比较的口径);③ 评测集标注 —— 新分工后数据集责任跟着 Agent 走:
> 规范文档与 DXF 样例归队友,30 张标注照片归你。前两件在你手上,建议 W2 第一天做完。
