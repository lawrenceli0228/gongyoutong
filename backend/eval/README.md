...
🔴 **这一套是「诊断套件」,命中率不参与红绿**(`SuiteSpec.diagnostic=True`)。

判据不是「分数低」,是**「同样的数据集、同样的代码,跑两次得两个不同的数」**。
2026-08-22 四次实测:**85.0% / 71.4% / 71.4% / 80.0%,而且每次红的行都不一样**。
原因是它跑**多跳**链 —— 每一跳的输出喂给下一跳,小差异逐跳放大;
而 routing 只判第一跳、一次调用定胜负,所以那套的数是稳的。

一把刻度不稳的尺子当门槛用**比没有尺子更坏**:第一次假红就会让人开始忽略它的红,
而它照出来的恰恰是最难发现的那类 —— 第 4 跑照出
`schedule>schedule>schedule>schedule`(同一个人被连派四次直到撞熔断)。

**所以看这套的方式是:看逐条明细,别看总分。**

⚠️ 两条不许「顺手统一」:
- **样本量硬闸对它照样判 FAIL**。命中率会抖,但「数据集被掏空」是确定性的 ——
  删到只剩 3 行时报告不该是一片中性色的「诊断」。
- **要摘掉 diagnostic 标记,得先拿出真方差的数据**。而量真方差必须跑**独立进程**:
  同一个进程里连跑 N 轮量到的是**缓存回放** —— 2026-08-22 那次三轮拿到完全相同的
  结果、极差 0.0%,而三轮耗时是 226s / 35s / 33s。
...
>
> ⚠️ **`clarify` 与 `fail` 在「两边路径都空」时判分**不作区分****(2026-08-22 定)。
> 区分它们靠的是「末尾有没有问号」—— 很粗:supervisor 说「这事我做不了,你是要我
> 记一条任务吗?」既是拒绝也带问号。真要分开得上外部 LLM 裁判读那句话的意图。
> 在那之前这套能可靠回答的只有**「派没派活」**。数据集里 `expected_status` 仍照实写
> (对读数据集的人有价值),只是判分不拿它当硬判据。**这是明知的精度损失,不是漏了。**
>
