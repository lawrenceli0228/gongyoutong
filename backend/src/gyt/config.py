...
class Settings(BaseSettings):
    ...
    #
    # 🔴 **2026-08-22 从 8 提到 12,依据是实测,别凭「8 够用」改回去。**
    # 起因:supervisor 那节「没选工地就问一句、把清单摆给他」的提示词落地之后,
    # 一次正常对话比原来**多一个 supervisor 回合**(问 → 等答 → 再派),而 8 步不够。
    # 同一组六句「工地有哪些图纸」的问法实测(每句只跑一次,刻意用六个不同问法 ——
    # 同一句跑六遍会命中 GytDiskCache,量到的是缓存回放不是六个样本):
    #     旧提示词 + 8      熔断 0/6,但只有 1/6 真的把工地清单摆出来(其余在支人去点界面)
    #     新提示词 + 8      熔断 **2/6**,摆清单 5/6
    #     新提示词 + 12     熔断 **0/6**,摆清单 **6/6**
    # 中间试过靠改措辞压熔断(去掉一条与名册冲突的指令),只从 2/6 动到 1/6 ——
    # 而且红的换成了另一句,是噪声级移动。这正是本仓那条红线:
    # **「提示词只是概率性生效,结构件才兜得住」**,所以最后动的是这个数。
    #
